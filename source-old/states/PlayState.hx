...
class PlayState extends MusicBeatState
{
	...
	override public function create()
	{
		...
		downBlackBar = new FlxSprite(0, FlxG.height - 50).makeGraphic(1,1,FlxColor.BLACK);
		downBlackBar.scrollFactor.set();
		downBlackBar.scale.set(FlxG.width, 50);
		downBlackBar.updateHitbox();
		downBlackBar.cameras = [camHUD];
		add(downBlackBar);

		upBlackBar = new FlxSprite(0, 0).makeGraphic(1,1,FlxColor.BLACK);
		upBlackBar.scrollFactor.set();
		upBlackBar.scale.set(FlxG.width, 50);
		upBlackBar.updateHitbox();
		upBlackBar.cameras = [camHUD];
		add(upBlackBar);
		...
		healthBar.bg.updateHitbox();

		healthBar.leftBar.updateHitbox();
		healthBar.rightBar.updateHitbox();
		...
		shaderhealthP1Txt = new FlxRuntimeShader(File.getContent('assets/shaders/coolGradient.frag'));
		...
		scoreTxt = new FlxText(0, healthBar.y + 25, FlxG.width, "", 20);
		scoreTxt.setFormat(Paths.font("vcr.ttf"), 20, FlxColor.WHITE, CENTER, FlxTextBorderStyle.OUTLINE, FlxColor.BLACK);
		scoreTxt.scrollFactor.set();
		scoreTxt.borderSize = 1.25;
		scoreTxt.visible = !ClientPrefs.data.hideHud;

		shaderscoreTxt = new FlxRuntimeShader(File.getContent('assets/shaders/coolGradient.frag'));
	...
	public dynamic function updateIconsPosition()
	{
		...
		iconP1.x = healthBar.bg.x + (healthBar.bg.width * (1 - healthBar.percent * 0.01)) + (150 * iconP1.scale.x - 150) / 2 - iconOffset;
		iconP2.x = healthBar.bg.x + (healthBar.bg.width * (1 - healthBar.percent * 0.01)) - (150 * iconP2.scale.x) / 2 - iconOffset * 2;*/
